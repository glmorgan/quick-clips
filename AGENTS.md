...
**Transform picker:** `src/picker.ts` renders the hold-to-configure list as a command-palette-style page served on an ephemeral `127.0.0.1` port and shown in a Chromium `--app=` window. It supports icons, live previews of each transform against the current clipboard, type-to-filter with match highlighting, and keyboard nav.
